<?php

namespace App;

use Illuminate\Database\Eloquent\Model;
use App\Models\Customer;
use App\Product;
use App\ProductOption;
use App\ProductCustomersGroup;
use App\Order;
class OrderProduct extends Model
{
    //
  
    public function product(){
        return $this->belongsTo(Product::class, 'id_product');
    }

    public function option(){
        return $this->belongsTo(ProductOption::class, 'product_options_id');
    }

    public function advancedPrice(){
        $order = Order::find($this->id_order);
        $customer = Customer::find($cart->id_customer);
        $product = Product::find($this->id_product);
        if($customer!=null){
            $prodcustgrp = ProductCustomersGroup::where('id_product',$this->id_product)
            ->where('id_customer_group',$customer->id_customer_group)
            ->where('qty','<',$this->quantity)
            ->orderBy('qty','Desc')
            ->limit(1)->get();
        }else{
            $prodcustgrp =[];
        }
     
        
     
        $specialprice=$product->price;
        
        if(($product->special_price!=null)&&($product->special_price!=0)){
            if($specialprice > $product->special_price){
                 $specialprice=$product->special_price;
            }
        }
        if($this->product_options_id!=null){
          
            $option = ProductOption::find($this->product_options_id);
            if($specialprice > $option->price){
                 $specialprice=$option->price;
             }
        }
        if(count($prodcustgrp)>0){
            if($specialprice > $prodcustgrp[0]->price){
              $specialprice=$prodcustgrp[0]->price;
            }
        }
       
        return $specialprice;
    }

    public function DiscountTicket(){
    try {
        $order = Order::find($this->id_order);
        $qtdprod=0;
        $discount=0;
     
        foreach($cart->products as $prod){
     
            
          
         
            if(($order->ticket->products()->pluck('products.id')->contains($prod->id))||($this->collectioninticket($prod->id))){
                $qtdprod = $qtdprod+1;
            }
        }

        
        if($order->id_ticket!=null){
           
            if(($order->ticket->products->pluck('id')->contains($this->product->id))||($this->collectioninticket($this->product->id))){
                
              if($order->ticket->discount_method_id==2){
                $discount=$cart->ticket->discount/$qtdprod;
               
              }
              if($order->ticket->discount_method_id==1){
               
                $discount=$this->advancedPrice()*$this->quantity * $order->ticket->discount/100;
             
              }   
            }
        }
      
        return $discount;
    } catch (\Throwable $th) {
        //throw $th;
     
    }
       
    }
    public function FinalPrice(){
        $final = (($this->quantity*$this->advancedPrice()) - $this->DiscountTicket()) /$this->quantity;
        return $final;
    }
    public function collectioninticket($id){
        $product = Product::find($id);
        $order = Order::find($this->id_order);
        $collinticket =false;
      
        if(count($order->ticket->collections)>0){
            foreach($product->collections as $collection){
                if($order->ticket->collections->pluck('id')->contains($collection->id)){
                    $collinticket =true;
                    break;
                }
            }
        }

        return $collinticket;
    }

    public function optiondescription(){
        $description="";
      if($this->product_options_id!=null){
        $option = ProductOption::find($this->product_options_id);
        
        $description = $option->descricao();  
     }
      
      

        return rtrim($description,'/');
      
      
    }

}

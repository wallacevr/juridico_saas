		...
		@if($errors->any())
			...
			<div class="row">
					<div class="col-4">
							...
							@php
								$clientesselecionados = $processo->clientes->pluck('id')
							@endphp
						...
						@foreach($clientes as $cliente)
							<option value="{{$cliente->id}}" 
								@if( $processo->clientes->pluck('id')->contains($cliente->id))
									selected
								@endif
									>{{$cliente->nome}}</option>
						...
						<div class="col-2">
								...
								<select class="js-example-basic-multiple form-select" name="qualiclientes" >
									@foreach($qualificacoes as $qualificacao)
										...
										@if(count($processo->clientes)>0)
											@if( $qualificacao->id == $processo->clientes[0]->pivot->qualificacao_id)
												selected
											@endif
										@endif	
										>{{$qualificacao->descricao}}</option>
				...
				<div class="row">
						...
						<div class="col-2">
								...
								<select class="js-example-basic-multiple form-select" name="qualienvolvidos" >
									@foreach($qualificacoes as $qualificacao)
										<option value="{{$qualificacao->id}}" 
											@if(count($processo->envolvidos)>0)
													@if( $qualificacao->id == $processo->envolvidos[0]->pivot->qualificacao_id)
														selected
													@endif
											@endif
										>{{$qualificacao->descricao}}</option>
				...
				<div class="row">
						<div class="col-6">
								...
								<input type="text" class="form-control" name="titulo" placeholder="Título do Processo" value="{{$processo->titulo}}"></input>
						</div>
				</div>
				<div class="row">
						<div class="col-2">
							...
							@foreach($instancias as $instancia)
								<option value="{{$instancia->id}}"
									@if($processo->instancias!=null)
													@if( $instancia->id == $processo->instancia_id)
														selected
													@endif
											@endif
								
									>{{$instancia->descricao}}</option>
				...
				<div class="row">
						<div class="col-2">
							...
							<input type="text" class="form-control" name="juizo" placeholder="Nº" value="{{$processo->juizo}}"></input>
						</div>
						<div class="col-2">
							...
							<select class="form-select" name="vara_id" >
								@foreach($varas as $vara)
									...
									@if($processo->vara_id!=null)
											@if( $vara->id == $processo->vara_id)
												selected
											@endif
									@endif
						...
						<div class="col-2">
							...
							<select class="form-select" name="foro_id" >
								@foreach($foros as $foro)
									...
									@if($processo->foro_id!=null)
											@if( $foro->id == $processo->foro_id)
												selected
											@endif
									@endif
																		
									>{{$foro->descricao}}</option>
							...
							<select class="js-example-basic-multiple form-select" name="acao_id">
								@foreach($acoes as $acao)
									<option value="{{$acao->id}}"
											@if($processo->acao_id!=null)
													@if( $acao->id == $processo->acao_id)
														selected
													@endif
											@endif
				...
				<div class="row">
						<div class="col-6">
								...
								<input type="text" class="form-control" name="linktribunal" placeholder="Digite o link do Tribunal" value="{{$processo->linktribunal}}"></input>
						</div>
				</div>
				<div class="row">
						<div class="col-6">
								<label for="objeto" class="form-label">Objeto</label>
								<textarea class="form-control" name="objeto" id="" cols="30" rows="5" placeholder="Digite a Descrição do processo">@php
									echo $processo->objeto;
									@endphp
								</textarea>
						</div>
				</div>
				<div class="row">
						<div class="col-2">
							...
							<input  type="number"   name="valorcausa" placeholder="Valor da Causa"  min="0" step="0.01" data-number-to-fixed="2" data-number-stepfactor="100" class="form-control currency" id="c2" value="{{$processo->valorcausa}}" />
						</div>
						<div class="col-2">
							<label for="dtdistribuicao" class="form-label">Data da Distribuição</label>
							<input  type="Date"   name="dtdistribuicao" class="form-control" value="{{$processo->dtdistribuicao}}" />
						</div>
						<div class="col-2">
							<label for="valorcondenacao" class="form-label">Valor da Condenação</label>
							<input  type="number"   name="valorcondenacao" placeholder="Valor da Condenação"  min="0" step="0.01" data-number-to-fixed="2" data-number-stepfactor="100" class="form-control currency" id="c2" value="{{$processo->valorcondenacao}}" />
						</div>
				</div>
				<div class="row">
						<div class="col-3">
							<label for="porcentagemhonoarios" class="form-label">% de Honorários</label>
							<input  type="number"   name="porcentagemhonorarios" placeholder="% de Honorários"  min="0" step="0.01" data-number-to-fixed="2" data-number-stepfactor="100" class="form-control currency" id="c2" value="{{$processo->porcentagemhonorarios}}" />
						</div>

						<div class="col-3">
							<label for="honorarios" class="form-label">Valor da Honorários</label>
							<input  type="number"   name="honorarios" placeholder="Valor dos Honorários (R$)"  min="0" step="0.01" data-number-to-fixed="2" data-number-stepfactor="100" class="form-control currency" id="c2" value="{{$processo->honorarios}}"  />
						</div>
				</div>
				<div class="row">
						<div class="col-6">
								<label for="observacoes" class="form-label">Observações</label>
								<textarea class="form-control" name="observacoes" id="" cols="30" rows="5" placeholder="Digite a Descrição do processo">@php
									echo $processo->observacoes;
									@endphp</textarea>
						</div>
				</div>
				<div class="row">
						<div class="col-6">
							...
							@foreach($users as $user)
								<option value="{{$user->id}}"
									@if( $processo->responsaveis->pluck('id')->contains($user->id))
										selected
									@endif

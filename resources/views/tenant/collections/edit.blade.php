...
@extends('layouts.tenant', ['title' => __("Update collection") . __(" - {$collection->name}")])
...
<div class="lg:grid lg:grid-cols-12 lg:gap-x-5">
    ...
    <div class="space-y-6 sm:px-6 lg:px-0 lg:col-span-8">
        <form id="collectionForm" action="{{ route('tenant.collections.update', $collection->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                ...
                <div class="bg-white py-6 px-4 space-y-6 sm:p-6">
                    <div class="grid grid-cols-3 gap-6">
                        <div class="col-span-12 sm:col-span-3">
                            @include('layouts.snippets.fields', ['type'=>'text','label'=>'Name','placeholder'=>'Collection name','name'=>'name','value'=> $collection->name ])
                        </div>
                        ...
                        <div class="col-span-3">
                            ...
                            <div class="mt-1">
                                <textarea id="description" name="description" rows="5" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm ">{{ old('description') ? old('description') :  $collection->description}}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="shadow sm:rounded-md sm:overflow-hidden">
                <div class="bg-white py-6 px-4 space-y-6 sm:p-6">
                    ...
                    <div class="col-span-12 sm:col-span-3">
                        @include('layouts.snippets.fields', ['type'=>'text','label'=>'Title','placeholder'=>'Collection title','name'=>'page_title','value'=> $collection->page_title ])
                    </div>
                    <div class="col-span-3">
                        ...
                        <div class="mt-1">
                            <textarea id="seo_description" name="seo_description" rows="5" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ old('seo_description') ? old('seo_description') :  $collection->seo_description  }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="col-span-3 sm:col-span-2">
                        ...
                        <div class="mt-1 rounded-md shadow-sm flex">
                            ...
                            <input type="text" name="slug" id="slug" autocomplete="slug" class="block w-full border border-gray-300 shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" value="{{ old('slug') ? old('slug') :  $collection->slug  }}" />
                            @error('slug')
                    ...
                    <div class="mt-4 space-y-4">
                        <div class="flex items-start">
                            <div class="h-5 flex items-center">
                                <input id="status" name="status" type="checkbox" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded" {{ old('status') || $collection->status ? 'checked' : '' }} value="1" />
                            </div>
                ...
                <div class="mt-1 border-2 border-gray-300 border-dashed rounded-md px-6 pt-5 pb-6 flex justify-center">
                    <div class="file-upload">
                        ...
                        <div id="file-upload-content" {{ $collection->image_url ? 'style=display:block;' : ''}}>
                            <img id="file-upload-image" src="{{  tenant_public_path() . '/images/collections/' .$collection->image_url }}" />

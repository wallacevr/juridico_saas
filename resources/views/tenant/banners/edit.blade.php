...
<div class="lg:grid lg:grid-cols-12 lg:gap-x-5">
    ...
    <div class="space-y-6 sm:px-6 lg:px-0 lg:col-span-8">
        <form id="bannerForm" action="{{ route('tenant.banners.update', $banner->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                ...
                <div class="bg-white py-6 px-4 space-y-6 sm:p-6">
                    <div class="grid grid-cols-3 gap-6">
                        <div class="col-span-12 sm:col-span-3">
                            @include('layouts.snippets.fields', ['type'=>'text','label'=>'Name','placeholder'=>'Banner name','name'=>'name','value'=> $banner->name ])
                        </div>
                        ...
                        <div class="col-span-12 sm:col-span-3">
                            @include('layouts.snippets.fields', ['type'=>'text','label'=>'URL','placeholder'=>'Banner URL','name'=>'url','value'=> $banner->url ])
                        </div>

                        <div class="col-span-3">
                            @include('layouts.snippets.fields', ['type'=>'text','label'=>'Title','placeholder'=>'Banner title','name'=>'title','value'=> $banner->title ])
                        </div>
                        ...
                        <div class="mt-1 border-2 border-gray-300 border-dashed rounded-md px-6 pt-5 pb-6 flex justify-center">
                            <div class="file-upload">
                                ...
                                <div id="file-upload-content" {{ $banner->image_url ? 'style=display:block;' : ''}}>
                                    <img id="file-upload-image" class="banner" src="{{  tenant_public_path() . '/images/banners/' .$banner->image_url }}" />
                                </div>
                            </div>
                        </div>
                        @error('image_url')
                    ...
                    <div class="mt-4 space-y-4">
                        <div class="flex items-start">
                            <div class="h-5 flex items-center">
                                <input id="status" name="status" type="checkbox" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded" {{ old('status') || $banner->status ? 'checked' : '' }} value="1" />
                            </div>
